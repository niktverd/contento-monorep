...
async function runMigrationsWithLogging() {
    ...
    try {
        // Load knex configuration from the project root regardless of where the script lives/was compiled to
        // Using process.cwd() guarantees that the path resolves correctly both in the source tree
        // and in the compiled dist version (e.g. dist/server/src/scripts).
        const knexfilePath = path.resolve(process.cwd(), 'knexfile');
        // eslint-disable-next-line @typescript-eslint/no-var-requires
        const dbConfig = require(knexfilePath)[environment];

    ...
    return new Promise((resolve, reject) => {
        ...
        ffmpeg()
            ...
            .on('start', (commandLine) => {
                workerLog.info(1, 'FFmpeg command: ' + commandLine);
            })
            .on('stderr', (stderrLine) => {
                workerLog.error(1, 'FFmpeg stderr:', stderrLine);
            })
            .on('error', (err) => {
                workerLog.error(1, 'Ошибка при обработке видео:', err);
            })
            .on('end', () => {
                ffmpeg()
                    .input(tempFilePath)
                    ...
                    .on('start', (commandLine) => {
                        workerLog.info(2, 'FFmpeg command: ' + commandLine);
                    })
                    .on('stderr', (stderrLine) => {
                        workerLog.error(2, 'FFmpeg stderr:', stderrLine);
                    })
                    .on('error', (err) => {
                        workerLog.error(2, 'Ошибка при обработке видео:', err);
                        reject(err);
                    })
                    .on('end', () => {
                        workerLog.info(2, 'Обработка и склейка видео завершены.');
                        resolve();
                    })
                    .run();
            })
